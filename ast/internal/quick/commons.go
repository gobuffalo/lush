...
var (
	...
	CALL   = NewCall(IDENT, NewIdent("Bar"), ast.Nodes{INT, FLOAT, STRING}, BLOCK)
	IF     = NewIf(nil, ast.True, BLOCK, nil)
	ELSE   = NewElse(BLOCK)
	ELSEIF = NewElseIf(IF)
	FOR    = NewFor(ARRAY, []ast.Ident{NewIdent("i"), NewIdent("n")}, BLOCK)
	RANGE  = NewRange(ARRAY, []ast.Ident{NewIdent("i"), NewIdent("n")}, BLOCK)
